#!/usr/bin/env python3
"""
Inverted inference script - background robustness test.
Runs the trained Dreamer agent in the Pong environment with inverted pixel values.
Tests whether the model is robust to background changes.
"""
import os
import pathlib
import pickle
...
def invert_pixels(obs):
    """
    Invert pixel values: inverted = 255 - original
    Works for both grayscale and RGB images.

    Args:
        obs: Observation array, either [0, 1] normalized or [0, 255] uint8

    Returns:
        Inverted observation in the same format as input
    """
    if obs.dtype == np.uint8:
        # If uint8, directly invert
        return 255 - obs
    else:
        # If float [0, 1], invert and keep normalized
        return 1.0 - obs


def main():
    print("=" * 80)
    print("DreamerV2 Inverted Inference - Background Robustness Test")
    print("=" * 80)
    print()
    ...
    print("⚠ PIXEL INVERSION ENABLED")
    print("  All observations will be inverted: pixel_value -> (255 - pixel_value)")
    print()

    # Run episodes and collect rewards
    print(f"Running {num_episodes} episodes with inverted pixels...")
    print("-" * 80)
    ...
    for episode_idx in tqdm(range(num_episodes), desc="Episodes"):
        ...
        while not done:
            # INVERT PIXELS before feeding to agent
            obs_inverted = invert_pixels(obs['image'])

            # Get action from agent using inverted observation (add batch dimension)
            obs_dict = {}
            for k, v in obs.items():
                if isinstance(v, (int, float, bool)):
                    ...
                    obs_dict[k] = v[None]
            obs_dict['image'] = obs_inverted[None]  # Replace with inverted image
    ...
    print("=" * 80)
    print("INVERTED INFERENCE RESULTS")
    print("=" * 80)
    ...
    output_path = "experiment3_inverted_results.pkl"
