...
class WorldModel(common.Module):
  ...
  def imagine(self, policy, start, is_terminal, horizon):
    ...
    for _ in range(horizon):
      ...
      # NOTE: Frame-saving code commented out due to causing infinite loop in tf.function
      # The file I/O operations were being executed during graph tracing, not execution
      # If you need to save imagined frames, implement it outside the tf.function context
...
class ActorCritic(common.Module):
  ...
  def __init__(self, config, act_space, tfstep):
    ...
    act_size = act_space.n if discrete else act_space.shape[0]
    self.actor = common.MLP(act_size, **self.config.actor)
