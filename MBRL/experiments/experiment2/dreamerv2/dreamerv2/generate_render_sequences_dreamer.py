...
def main():
    ...
    # Collect multiple sequences and concatenate them
    all_obs = []
    all_actions = []

    min_steps_needed = 250  # Need at least 250 steps for 10 sequences of length 20

    while len(all_obs) == 0 or sum(len(o) for o in all_obs) < min_steps_needed:
        ...
        obs = obs_tensor[0].numpy()  # (time, H, W, C)
        action = action_tensor[0].numpy()  # (time, action_dim)
    ...
    print(f"✓ Loaded test data with {obs_array.shape[0]} steps from {len(all_obs)} episodes")
    print(f"   Observation shape: {obs_array.shape}")
    print(f"   Action shape: {action_array.shape}")
    print()

    # Convert observations to grayscale 84x74 (crop score area)
    def obs_to_grayscale_84x74(obs_hwc):
        """Convert observation to 84x74 grayscale (cropped to remove score)."""
        # Squeeze out single channels if present: (H, W, 1) -> (H, W)
        if obs_hwc.shape[-1] == 1:
            obs_hwc = obs_hwc.squeeze(-1)

        # Convert to uint8
        img_uint8 = (obs_hwc * 255).astype(np.uint8)

        # Create PIL Image (handle both grayscale and RGB)
        if len(img_uint8.shape) == 2:
            img = Image.fromarray(img_uint8, mode='L')  # Grayscale
        else:
            img = Image.fromarray(img_uint8)  # RGB
            img = img.convert('L')  # Convert to grayscale

        # Resize to 84x84
        img_resized = img.resize((84, 84), Image.BILINEAR)
    ...
    for start_idx in tqdm(start_indices, desc="Generating sequences"):
        ...
        # Convert real frames to 84x74 grayscale (cropped)
        real_frames = []
        for step in range(sequence_length + 1):
            frame = obs_to_grayscale_84x74(real_sequence[step])
        ...
        # Convert initial frame to 84x74 grayscale (cropped)
        initial_frame = obs_to_grayscale_84x74(initial_obs)
        ...
        context_data = {
            ...
            'is_first': tf.convert_to_tensor(np.array([[False]]), dtype=tf.bool),  # (1, 1)
        }

        # Encode initial observation to latent state
        embed = agent.wm.encoder(context_data)
        post, prior = agent.wm.rssm.observe(embed, context_data['action'], context_data['is_first'])
        ...
        for step in range(sequence_length):
            ...
            real_action = action_array[start_idx + step]  # (action_dim,)
            action_tensor = tf.convert_to_tensor(real_action[None], dtype=tf.float32)  # (1, action_dim)

            # Imagine next latent state
            latent = agent.wm.rssm.img_step(latent, action_tensor)

            # Get feature representation
            feat = agent.wm.rssm.get_feat(latent)

            # Decode latent to image
            decoder_out = agent.wm.heads['decoder'](feat)
            if isinstance(decoder_out, dict):
                pred_image_dist = decoder_out['image']
            else:
                pred_image_dist = decoder_out
            pred_image = pred_image_dist.mode().numpy()[0]  # Remove batch dimension

            # Convert predicted frame to 84x74 grayscale (cropped)
            pred_frame = obs_to_grayscale_84x74(pred_image)
    ...
    print()
    print(f"Real frames shape (84x74 grayscale, cropped): {all_real_frames.shape}")
    print(f"Predicted frames shape (84x74 grayscale, cropped): {all_predicted_frames.shape}")
    print()
    ...
    print("=" * 80)
    print("Mean MSE per Horizon Length (84x74 Cropped Grayscale Pixel Space)")
    print("=" * 80)
