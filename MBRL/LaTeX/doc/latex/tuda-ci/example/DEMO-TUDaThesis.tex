...
involves alternating between updating the \worldmodel{} and optimizing the policy
...
robustness. Combining both appraoches thus offers the possibility of complex
planning using object centric states. Although out of the scope of this thesis
one could imagine interpretable planned stategies describing how certain
...
the \worldmodel{} architecture, actor-critic integration, and training pipeline.
...
that is sampled by the newly trained agent in the real
...
example the \worldmodel{} can introduce a bias into the agent which does not hold
...
\cite{hafner2019learninglatentdynamicsplanning} and models from  \cite{ha2018world}, which first used latent
...
are different ways to train a \worldmodel{}. You can predict future observations
directly or predict latent states. In Dreamers case as shown in Figure
\ref{fig:dreamer_flow} the \worldmodel{} predicts latent states.

DreamerV2 builds upon the \worldmodel{} with an actor-critic framework. The actor
...
relational structure into \worldmodel{}s, agents can make more accurate
predictions about future states and plan more effectively. This integration
forms a core component of our proposed approach, where object-centric \worldmodel{}s
explicitly encode relational information to enhance both prediction
...
\section{\worldmodel{} Architecture}
...
Our complete training procedure alternates between the \worldmodel{} training and
the policy optimization, with periodic retraining of the \worldmodel{} using
fresh experience collected by the improving policy. This cyclic approach is in
many instances of model based RL incredible important since the \worldmodel{}
...
\subsection{\worldmodel{} Training and Experience Collection}
...
ensure the \worldmodel{} learns from the latest distribution of states visited by
...
\section{\worldmodel{} Prediction Accuracy}
...
overfitting to \worldmodel{} errors. The 7-step imagination horizon is sufficient
for effective policy learning with appropriate temporal credit assignment.
...
makes faster learning possible, and explicit object features providing natural
...
game entities. This hints into the direction of object-centric approaches:
agents that operate on symbolic object representations (paddle positions, ball
...
assuming proper object extraction.
...
performance without overfitting to \worldmodel{} errors. This shows that standard
actor-critic methods work well with object-centric \worldmodel{}s without
requiring specialized architectural modifications.
...
without extensive exploration. Also, we have simplified credit assignment
through transparent causal chains from actions to outcomes.
...
exclusively in Pong, which is a relatively simple domain with limited object
types (2 paddles, 1 ball), simple physics, and small state space.
Generalization to more complex Atari games (Breakout with many bricks, Seaquest
with multiple fish types) or domains with stochastic elements, partial
observability, and complex multi-object interactions remains uncertain. Pong's
continuous dynamics may be particularly favorable for object-centric
prediction. Games with discrete object appearances (spawning, destruction) may
experience larger prediction errors with significant downstream consequences.
Games like Seaquest in JAXAtari represent object appearances by choosing some
values to not be Zero. This means that the difference between an object being
present or not is just a matter of a single bit in the representation. This
makes it challenging to predict accurately and especially leads to the same
error in case of a misprediction as the object being moved a single pixel left
or right which is clearly not an error of the same magnitude.
...
benefit from long-term planning over tens or hundreds of steps. This requires
future work on learned error correction, probabilistic prediction with
uncertainty quantification, or hierarchical \worldmodel{}s. This is not a
...
DreamerV2-style actor-critic successfully adapted to object-centric \worldmodel{}s.
It learns entirely from simulated rollouts and demonstrates
...
discovery methods (slot attention, MONet), uncertainty-aware prediction,
