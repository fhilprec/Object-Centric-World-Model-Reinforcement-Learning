...
\usepackage{float}
\usepackage{pdfpages}
% hyperref is loaded automatically by pdfx when pdfa=true is set in document class
...
\author[Florian Hilprecht]{Florian Hilprecht}
\studentID{2797159}
\reviewer{Prof. Dr. Kristian Kersting\\[0.2em]2. Review: Jannis Blüml \\[0.2em]3. Review: Raban Emunds}
...
\includepdf[pages=-]{signedPage2.pdf}
...
or games especially well without grasping the underlying rules of the game.
Unlike supervised learning for example there is no ground truth from which
the agent can learn from. Among numerous distinctions within reinforcement
learning there is the difference between model-based reinforcement learning and
...
model-free approaches in particular are notoriously data hungry often requiring
...
\cite{DBLP:conf/rlc/DelfosseBGSK24} provides a standardized framework for extracting
object representations from Atari environment. This makes
different object-centric approaches more comparable.

Object centricity has the goal of interpretability and robustness whereas model-based approaches try to improve sample efficiency and planning capabilities
...
paradigms. Therefore it is guided by the following research questions:
\begin{enumerate}
	...
	\item What is the impact of different \worldmodel{} architectures (such as LSTM-based
	      versus alternative designs) on the overall performance of object-centric model-based reinforcement learning in structured environments?
	\item Does the object-centric inductive bias in \worldmodel{}s improve robustness to
	      changes in the environment?
\end{enumerate}

Within this thesis we propose a \worldmodel{} that is running on top
of object-centric states. This \worldmodel{} is trained to predict the next state of the
environment given the current state and action. An actor-critic framework based
on the DreamerV2 \cite{DBLP:conf/iclr/HafnerLB020} architecture is integrated with the
...
The main advantage of object-centric \worldmodel{}s over pixel-based alternatives
is the combination of planning capabilities with interpretability. While
pixel-based \worldmodel{}s plan in abstract latent spaces that may hide 
the reasoning of the agent, object-centric \worldmodel{}s enable complex planning
directly in the space of meaningful entities (paddles, balls, obstacles). This
allows for fundamentally more transparent decision-making: it is possible to trace how
specific objects and their predicted trajectories influence the agent's
actions. Although detailed interpretability analysis is beyond the scope of
this thesis, this approach opens possibilities for explainable strategies where
the agent's reasoning about object interactions can be made human-interpretable.
Such insights are typically unattainable with existing pixel-based
model-based approaches, which rely on opaque latent representations optimized
for reconstruction rather than semantic understanding.
...
\cite{DBLP:conf/icml/DuanCHSA16, DBLP:conf/iclr/KaiserBMOCCEFKL20}. Also, the lack of a model can make
...
tasks and make the agent more robust \cite{DBLP:conf/nips/HaS18,DBLP:conf/iclr/HafnerLB020}.
...
\cite{DBLP:conf/nips/JannerFZL19,sutton1991dyna}. Moreover there are approaches that use
...
\cite{schrittwieser2020mastering,DBLP:conf/icml/GreydanusKDF18}.
...
The Dreamer architecture \cite{DBLP:conf/iclr/HafnerLB020} is a family of model-based RL algorithms (DreamerV1,
V2, V3) that use a \worldmodel{} together with actor-critic methods \cite{barto1983neuronlike}. The main
...
\cite{DBLP:conf/icml/HafnerLFVHLD19} and models from
\cite{DBLP:conf/nips/HaS18}, which first used latent dynamics, but Dreamer adds
actor-critic learning and works better for Atari games.

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.9\textwidth]{images/DreamerFlow.png}
	\caption[DreamerV2 training pipeline overview]{Overview of the DreamerV2 training pipeline. The \worldmodel{} learns to predict future states and rewards, enabling the actor-critic framework to optimize policies using imagined rollouts. Taken from Hafner et al.~\cite{DBLP:conf/iclr/HafnerL0B21}, Fig.~1.}
...
While DreamerV2 is using pixel-based observations,
our work differs in several ways. Instead of learning latent
representations from 84×84 pixel observations through convolutional encoders
and the RSSM architecture, we leverage explicit object-centric states extracted
from JAXAtari. This eliminates the need for representation learning entirely
and allows us to use a simpler feedforward architecture rather than recurrent
models. Additionally, where DreamerV2 learns to predict rewards through the
\worldmodel{}, we utilize geometric reward functions based on object positions.
...
input to create objects from it \cite{DBLP:conf/nips/LocatelloWUMHUD20} but also approaches
where the objects states are extracted from RAM \cite{DBLP:conf/rlc/DelfosseBGSK24} or
...
understands the concept of a "ball" an an abstract object can use this knowledge in environments where the ball has a different shape or appearance in general \cite{DBLP:conf/iclr/ZambaldiRSBLBTR19}.
...
pixel data, agents can learn policies with fewer rollouts. This is possible because the object space is already a kind of latent space. \cite{DBLP:conf/nips/LocatelloWUMHUD20}.

A significant development in this field is the introduction of OCAtari
(object-centric Atari) by Delfosse et al. \cite{DBLP:conf/rlc/DelfosseBGSK24}. OCAtari
...
\cite{DBLP:conf/rlc/DelfosseBGSK24}. This misalignment problem strengthens the argument
for using object-centric understanding rather than attempting to retrofit
interpretability onto pixel-based systems.

Building upon OCAtari, JAXAtari \cite{jaxatari} provides a
...
\subsection{Object-Centric World Models for Reinforcement Learning}

While object-centric representations and model-based RL have each been studied
extensively, their combination remains an active area of research. Several
recent works have demonstrated the benefits of object-centric world models.
Zhang et al. \cite{DBLP:journals/corr/abs-2501-16443} show that object-centric world models
improve reinforcement learning in visually complex environments, demonstrating
better generalization and sample efficiency compared to pixel-based
alternatives. Mosbach et al. \cite{DBLP:conf/icml/MosbachEVB25} learn object-centric
latent dynamics directly from pixels for model-based RL, combining
representation learning with dynamics prediction.

Most of these approaches learn object representations jointly with dynamics
models from pixel observations \cite{DBLP:conf/iclr/KipfEMSSHJDG22,villar2023object}.
While this end-to-end learning is more general, it introduces additional
complexity and potential failure modes. This thesis takes another approach.
By using ground-truth object-centric states from JAXAtari, we can isolate the
benefits of object-centric dynamics modeling from the challenge of object
discovery. This allows us to provide insights for both learned and extracted object-centric approaches.

The integration of these three components—model-based RL for sample efficiency,
the Dreamer architecture for policy learning from imagination, and
object-centric representations for structured understanding—forms the
foundation of our approach. While each has been studied independently, their
combination presents a promising but unexplored direction for achieving both
sample efficiency and interpretability in reinforcement learning. The following
chapter details our implementation that brings these components together.

\chapter{Methodology}
\label{chap:methodology}

In this chapter we explore the actual implementation of the \worldmodel{}
...
\cite{DBLP:conf/iclr/HafnerLB020} described in Section \ref{sec:dreamer_architecture}.
...
Figure~\ref{fig:pong_architecture} illustrates the complete training pipeline:
JAXAtari transforms pixel observations into object-centric states, which
the dynamics MLP uses to predict future states over a 7-step horizon (shown as
2 steps for clarity). The actor samples actions while the critic provides value function
estimates, with both networks trained through losses calculated from the return
computation. The following sections describe each component of this architecture.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{images/Pong.jpg}
	\caption[Pong World Model architecture]{Overview of the Pong World Model architecture. The model takes in the current object-centric state and action, processes them through a deep residual MLP, and predicts the next object-centric state.}
	\label{fig:pong_architecture}
\end{figure}

\section{World Model Architecture} \label{sec:world_model_arch}

Unlike DreamerV2 we use the fact that we already
have object centric states. So instead of a 84 by 84 Pixel image as input we have
a 12-dimensional feature vector per frame. Note that while the full
\texttt{PongObservation} contains 14 features (including scores), we only use
12 features for the \worldmodel{} which will be explained in Section
\ref{sec:exp_setup}. Each object-centric state in Pong is represented as a
\texttt{PongObservation} containing:
\begin{itemize}
...
Using a simple feedforward network has several advantages. First, they are
significantly faster to train and evaluate than recurrent models, since they
have no sequential dependencies that require iterative computation.
Also using them during training becomes easier
since feeding them a single frame is sufficient which allows for better
parallelization. Second, the structured nature of object-centric states
(explicit positions) means the model does not need to learn temporal
abstractions from scratch. The main disadvantage of
feedforward models is that they lack temporal context which is solved to some
degree by the frame stacking.
...
to improve the learning stability in actor-critic methods \cite{DBLP:conf/nips/YueWZ20}.
...
\begin{figure}[ht]
	...
	\caption[Training schedule phases]{Training schedule showing the alternating phases of \worldmodel{} training and policy optimization.}
	\label{fig:trainingschedule}
\end{figure}

\subsection{World Model Training and Experience Collection} \label{subsec:world_model_training}
...
The training pipeline begins with initially collection experience using a predefined
heuristic policy, followed by alternating phases of \worldmodel{}
...
When retraining, we completely resample transitions from the updated buffer to
...
approach, we use JAXAtari \cite{jaxatari} to extract states. This gives us direct access to
...
normalization statistics. Our architecture uses a 4-layer deep residual MLP as
the dynamics model that predicts single-frame transitions from 4-stacked frames,
combined with separate actor and critic networks that learn entirely from
7-step imagined rollouts generated by this \worldmodel{}.
...
predictions to 84×84 greyscale images to make comparison fair. Sample
...
and three seeds for the object-centric approach.

\textbf{Experimental Scope:} While our primary evaluation focuses on the three
experiments detailed in this chapter (prediction accuracy, policy learning, and
robustness), the development process involved additional exploratory
experiments. These included investigations into different network architectures
(varying depth and width), alternative frame stack sizes (2, 4, 8 frames),
reward prediction approaches, and various hyperparameter configurations for the
actor-critic training \cite{DBLP:conf/icml/YoonWBA23}. The configurations
presented here represent the most stable and well-performing setups identified
through this exploratory process.

\section{World Model Prediction Accuracy} \label{sec:world_model_perf}
...
\begin{figure}[ht]
	...
	\caption[World model prediction error comparison]{\worldmodel{} prediction error comparison over multi-step rollouts. The JAX object-centric \worldmodel{} (orange) exhibits substantially lower MSE compared to DreamerV2 (blue) across all prediction horizons from 1 to 20 steps.}
...
respect Pong's physics. Although beneficial this should not be taken out of context
...
\begin{figure}[ht]
	...
	\caption[Training progress comparison]{Training progress comparison between object-centric (blue) and DreamerV2 (orange). The object-centric approach achieves high returns (extracted from real rollouts) with an order of magnitude fewer environment interactions.}
...
\begin{table}[ht]
	\centering
	\caption[DreamerV2 performance under visual perturbations]{DreamerV2 performance under normal and inverted visual conditions over 20 evaluation episodes.}
...
\begin{table}[ht]
	\centering
	\caption[Summary of experimental results]{Summary of key experimental results mapped to research questions.}
...
\begin{figure}[ht]
	...
	\caption[Training with and without feature weighting]{Comparing training with and without feature weighting. While both approaches eventually converge to similar performance levels, the variant without feature weighting shows slightly worse performance in the initial training phase before reaching comparable final scores.}
	\label{fig:feature_weighting_ablation}
\end{figure}

To validate the importance of feature weighting, we compared training with and without
 this design choice (Figure \ref{fig:feature_weighting_ablation}). The results show that while feature weighting is 
 not strictly essential for learning, it does provide benefits during early training. The agent trained without 
 feature weighting shows slightly worse initial performance but eventually converges to similar final scores. 
 This suggests that feature weighting helps guide the model to focus on the most critical features (ball trajectory) early 
 in training, accelerating convergence, though the agent can still learn effectively without it.
...
dynamics modeling may abstract away crucial information for reward prediction.
...
behaviors. Our feedforward MLP architecture has a fixed input size and does not
explicitly model object-object relationships, limiting its applicability to such
scenarios. Graph neural networks or attention mechanisms
\cite{carion2020end,DBLP:conf/nips/LocatelloWUMHUD20} might improve that by providing
permutation invariance and handling variable numbers of objects.
Complex interaction types (pickup, destruction, transformation) and 
partial observability (occlusion, object persistence) may require more sophisticated architectures than the
feedforward MLP we use here. This highlights important directions for future
research.
...
discovery methods (for example slot attention \cite{DBLP:conf/nips/LocatelloWUMHUD20}), uncertainty-aware prediction,
...
The author used large language models (ChatGPT), translation tools (DeepL Write), and grammar checkers (Grammarly) for language polishing and grammar correction during the preparation of this thesis. No content, results, or scientific arguments were generated by these tools.
